...
var viewTableCreator=function(){
  ...
  var myMousemove= function(e){
    ...
    if(iCur>0) {  //Check if previous is better
      var rowPrev=movedRow.previousElementSibling;
      var yPrevOff=rowPrev.getBoundingClientRect().top;
      var hPrev=rowPrev.offsetHeight;
      if(y<yPrevOff+hPrev/2) { rowPrev.insertAdjacentElement('beforebegin', movedRow); boMoved=true }
    }
    if(!boMoved && iCur<len-1) { //Check if next is better
      var rowNext=movedRow.nextElementSibling;
      var yNextOff=rowNext.getBoundingClientRect().top;
      var hNext=rowNext.offsetHeight;
      if(y>yNextOff+hNext/2) { rowNext.insertAdjacentElement('afterend', movedRow); }
    }
  ...
  el.contains=function(uri){ return el.getRowByURI(uri)!==null;}

  // el.setUUID=function(r, strUUID){
  //   r.persistData.strUUID=strUUID;
  // }
  ...
  el.sendAllOn=function(pos){
    ...
    sendAllMess({boShow:true, hideTimer, lat, lng, boSetTHide:true});
  }
...
var viewAddPopCreator=function(){
  ...
  var save=function(){
    ...
    var row=viewTable.getRowByURI(uri);
    var iRole=Number(inpRole.value);
    // if(row===null) { setMess('That uri is already in the list', 2); return;  } 
    if(row===null) { 
      ...
      var strUUIDOld=row.persistData.strUUID
      row.persistData.strUUID=strUUID;
      var strMess=(strUUIDOld==strUUID)?'same key':'key changed';
      setMess(strMess, 2);
      var iRow=row.myIndex();
      if(boNativeAndroid) Android.setKey(iRow,strUUID); else setItems();  // Not tested with boNativeAndroid==true
    }
    doHistBack();
    //el.closePop();
  }

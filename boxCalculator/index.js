...
var headPerm=createElement('h3').myAppend(`Select alternative`)
var formPerm=createElement('form').css({ border:'1px solid', width:'fit-content', background:'var(--bg-colorEmp)'});
var LabPerm=[], InpPerm=[];
var br=createElement('br')
for(var i=0;i<3;i++){
  var lab=createElement('label'); //.css({background:'lightgrey', border:'1px solid', width:'fit-content'});
  var inp=createElement('input').attr({type:'radio', id:`${i}`, name:"permutationButts", value:`${i}`}).on('click', cbCalc); //.css({background:'lightgrey', border:'1px solid', width:'fit-content'})
...
var spanX=createElement('span').css({'margin-left':'6em'}), spanY=createElement('span');
[spanX, spanY].forEach(ele=>ele.css({background:'var(--bg-colorEmp)'}))
var divX=createElement('div').myAppend(spanX)


var img=createElement('img').prop({src:'cardbordBoxAlpha.png'}).css({'vertical-align':'middle'}).addClass('invertOnDarkBright')
// var figcaption=createElement('figcaption').myAppend(``)
// var fig=createElement('figure').myAppend(img,figcaption)

var divBreadB=createElement('div').myHtml(`Numbers in parentheses (in X and Y) should not be cut (only folded (so you may want to mark them differently)).<br>
Cut the solid lines, and fold the dotted lines.<br>
Glue the overlap-flap and put it together.`)
elBody.myAppend(pIntro, pDimensionless, ...El, spanErr, headPerm, formPerm, spanTip, headResult, divX, img, spanY, divBreadB)
cbCalc()

var aLink=createElement('a').attr({href:'https://gavott.com'}).myText('More from the same developer')
var headMore=createElement('h3').myAppend(aLink);
elBody.myAppend(headMore)

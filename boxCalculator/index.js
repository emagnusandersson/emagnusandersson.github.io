...
// Glue the overlap-flap and put it together.\n\n`)
  ...
  pre.myAppend(`Tip: Use the alternative with the smallest Area to reduce the required amout of cardboard.\n\n`)
}
...
var pIntro=createElement('pre').myAppend(`Enter the (inner) box dimensions in the fields below:
  w (width)
  d (depth)
  h (height)
  t (thickness of cardboard).
  xGlueOverlap (see image)
Note! the values are dimensionless (so consider the dimensions as mm, inch or whatever)`).css({'margin-right':'0.4em'})
...
elBody.myAppend(pIntro, ...El, butGo) //, spanUnit

var divBreadB=createElement('pre').myAppend(`Press "Go" and the calculated values of how to cut and fold a sheet of cardboard will appear below:
  X (left-to-right in the picture below)
  Y (down-to-top in the picture below)
X-/Y- numbers in parentheses should not be cut (only folded).
Cut the solid lines, and fold the dotted lines.
Glue the overlap-flap and put it together.\n\n`)

var pre=createElement('pre');
elBody.myAppend(divBreadB, pre)
